...
export function AcaiBoat() {
    ...
    //tamanho do copo
    const arraySize = data.boatSizes
    const sizeMap = arraySize.map((item) => {
        return { label: item, value: item }
    })
    const [size, setSize] = useState('')
    const [sizeList, setSizeList] = useState(sizeMap)
    ...
    useEffect(() => {
        ...
        setMaxFree(maxFree)
        let freeValue = free.length > maxFree ? (free.length - maxFree) * 3 : 0
        setTotalValue(parcialValue + (paid.length * 5) + freeValue)
    }, [size, syrup, fruit, free, paid, showCup, select])
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.title}>World Barca</Text>
            </View>
